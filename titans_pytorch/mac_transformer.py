...
class MemoryAsContextVisionTransformer(Module):
    @staticmethod
    def _to_pair(value: int | tuple[int, int]):
    ...
    def __init__(
        ...
        *,
        ...
        self.is_vision = True
        self.custom_token_emb = token_emb

        self.image_size = None
        self.vision_grid = None
        self.vision_num_patches = None

        patch_height, patch_width = self._to_pair(default(patch_size, 4))
        self.patch_size = (patch_height, patch_width)
        self.channels = channels

        if exists(image_size):
            image_height, image_width = self._to_pair(image_size)
            assert divisible_by(image_height, patch_height) and divisible_by(image_width, patch_width), 'image dimensions must be divisible by patch size'
            self.image_size = (image_height, image_width)
            self.vision_grid = (image_height // patch_height, image_width // patch_width)
            self.vision_num_patches = self.vision_grid[0] * self.vision_grid[1]

        if not exists(token_emb):
            patch_dim = channels * patch_height * patch_width
            self.image_token_emb = nn.Sequential(
                Rearrange('b c (h ph) (w pw) -> b (h w) (c ph pw)', ph = patch_height, pw = patch_width),
                nn.Linear(patch_dim, dim)
            )
            self.text_token_emb = nn.Embedding(num_tokens, dim)
        else:
            self.image_token_emb = None
    ...
    def embed_tokens(self, x):
        ...
        if exists(self.custom_token_emb):
            return self.custom_token_emb(x)

        if x.ndim == 4:
            ph, pw = self.patch_size
            height, width = x.shape[-2:]
            assert divisible_by(height, ph) and divisible_by(width, pw), 'image dimensions must be divisible by patch size'
            return self.image_token_emb(x)

        return self.text_token_emb(x)
    ...
    def sample(
        ...
        if self.is_vision:
            raise RuntimeError('sampling is only supported for autoregressive text inputs')
